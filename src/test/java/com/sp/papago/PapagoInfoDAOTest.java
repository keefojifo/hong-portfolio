...
//test할것을 앞에다가 적고 뒤에 test를 적는 것이 규칙이다. 
		...
		public void test() {
			//Assert.is
			Assert.notNull(pidao,"그럼 그렇치 ");
			//Assert.isNull(pidao,"null 에러");
			List<PapagoInfoVO> piList = pidao.getPapagoList(null);
			//Assert.isTrue(piList.size()!=1,"사이지가 1일리가 없지");
